...
const shelves = [
  {
    title: "Short, strange, unforgettable",
    description: "Novels under 200 pages",
    palette: ["#274333", "#b58247", "#6f4538"],
  },
  {
    title: "Classics that still feel alive",
    description: "Old stories, sharp edges",
    palette: ["#76513c", "#243b2d", "#be945b"],
  },
  {
    title: "A weekend in another world",
    description: "Immersive literary escapes",
    palette: ["#a46d3e", "#485e52", "#402f2b"],
  },
  {
    title: "Books about books",
    description: "For incurable readers",
    palette: ["#21362a", "#8f6446", "#c29a61"],
  },
] as const;
  ...
  return (
    <div className="mb-8 flex items-end justify-between gap-6">
      <div>
        ...
        <h2 className="font-serif text-3xl leading-tight tracking-[-0.025em] text-primary sm:text-4xl">
          {title}
        </h2>
        {description ? (
      ...
      {href && linkLabel ? (
        <Link
          href={href}
          className="hidden shrink-0 items-center gap-2 border-b border-accent/40 pb-1 text-sm font-semibold text-primary transition-colors hover:border-accent hover:text-accent sm:flex"
        >
          {linkLabel} <ArrowIcon />
        </Link>
      ) : null}
      ...
      {palette.map((color, index) => (
        <div
          ...
          style={{
            ...
            transform: `translateY(${index === 1 ? 0 : 12}px)`,
          }}
        >
...
export default function Home() {
  return (
    <main className="flex-1 overflow-hidden">
      <section
        id="search"
        className="relative isolate min-h-[620px] overflow-hidden bg-[#172019] text-white"
      >
        <Image
        ...
        <div className="mx-auto flex min-h-[620px] w-full max-w-7xl items-center px-5 py-20 sm:px-8 lg:px-10">
          <div className="w-full max-w-3xl">
            ...
            <p className="mt-6 max-w-2xl text-base leading-7 text-white/72 sm:text-lg">
              Search millions of stories, follow a curious thread, or start with
              a shelf made for the mood you are in.
            </p>
            ...
            >
              <label className="flex h-14 flex-1 items-center gap-3 rounded-full border border-white/35 bg-white/95 px-5 text-[#223023] shadow-2xl shadow-black/15 focus-within:ring-2 focus-within:ring-[#d7aa69]">
                ...
                <input
                  ...
                  className="min-w-0 flex-1 bg-transparent text-base outline-none placeholder:text-[#223023]/50"
                />
              </label>
              <button
                type="submit"
                className="h-14 rounded-full bg-[#d1a15f] px-7 text-sm font-bold text-[#172019] shadow-xl shadow-black/20 transition-colors hover:bg-[#e0b878] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
              >
                Search library
              </button>
            </form>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2.5 text-sm">
              ...
              {[
                ...
                <Link
                  ...
                  className="rounded-full border border-white/25 bg-black/10 px-3.5 py-1.5 text-white/80 backdrop-blur-sm transition-colors hover:border-white/55 hover:bg-white/10 hover:text-white"
                >
                  {label}
                </Link>
              ))}
      ...
      <section
        id="categories"
        className="border-b border-primary/10 bg-secondary py-18 sm:py-22"
      >
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-10">
          <SectionHeading
            eyebrow="Explore the catalog"
            title="Browse by subject"
            description="Begin broadly, then follow the details that catch your attention."
            href="/subjects"
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
            {subjects.map((subject, index) => (
              ...
              >
                ...
                <div>
                  <h3 className="font-serif text-lg text-primary group-hover:text-accent">
                    {subject.name}
                  </h3>
                ...
                <span
                  className={`mt-4 h-px bg-primary/15 ${index % 2 === 0 ? "w-10" : "w-16"}`}
                />
              </Link>
            ))}
          </div>
      ...
      <section
        id="discover"
        className="border-b border-primary/10 bg-white py-18 sm:py-22 dark:bg-secondary"
      >
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-10">
          <SectionHeading
            eyebrow="Curated by Avenor"
            title="Shelves worth starting from"
            description="Small, purposeful collections for the days when a blank search box feels like too much."
            href="/discover"
          ...
          <div className="-mx-5 flex snap-x gap-4 overflow-x-auto px-5 pb-4 sm:mx-0 sm:grid sm:grid-cols-2 sm:overflow-visible sm:px-0 lg:grid-cols-4">
            {shelves.map((shelf) => (
              <Link
                key={shelf.title}
                href="/discover"
                className="group w-[78vw] max-w-72 shrink-0 snap-start sm:w-auto sm:max-w-none"
              >
                <BookStack palette={shelf.palette} />
                <h3 className="mt-5 font-serif text-xl leading-6 text-primary transition-colors group-hover:text-accent">
                  {shelf.title}
                </h3>
        ...
        <div className="mx-auto grid w-full max-w-7xl gap-16 px-5 sm:px-8 lg:grid-cols-[1.15fr_.85fr] lg:gap-20 lg:px-10">
          <div>
            <SectionHeading
              ...
              description="Saved locally in this browser. No account wall, no interruption."
            />
            <div className="divide-y divide-primary/10 border-y border-primary/10">
              {recentBooks.map((book) => (
                ...
                >
                  <div
                    className="h-18 w-12 shrink-0 rounded-[2px_5px_5px_2px] border border-black/10 shadow-md"
                    style={{ backgroundColor: book.color }}
                  />
                  <div className="min-w-0 flex-1">
                    <h3 className="truncate font-serif text-base text-primary sm:text-lg">
                      {book.title}
                    </h3>
                  ...
                  <Link
                    href="/favorites"
                    className="text-xs font-bold text-accent hover:text-primary sm:text-sm"
                  >
                    Resume
                  </Link>
                </div>
          ...
          <div>
            <SectionHeading
              ...
              description="Move from one voice to the body of work around it."
            />
            <div className="grid grid-cols-2 gap-3">
              {authors.map((author, index) => (
                <Link
                  key={author.name}
                  href={`/discover?q=${encodeURIComponent(author.name)}&mode=author`}
                  className="group flex items-center gap-3 rounded-2xl border border-primary/10 bg-white p-3.5 transition-colors hover:border-accent/50 dark:bg-white/[0.035]"
                >
                  <span
                    className={`grid size-11 shrink-0 place-items-center rounded-full font-serif text-sm ${index % 2 === 0 ? "bg-primary text-secondary" : "bg-accent/18 text-primary"}`}
                  >
                    {author.initials}
                  </span>
                  <span className="text-sm font-semibold leading-5 text-primary/75 group-hover:text-primary">
                    {author.name}
                  </span>
                </Link>
              ))}
